# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 11:31:14 2019
...
print("tokenized the wine sentences") 
...
print("turned the tokenized sentences into indices")
seqs = [tokenizer.convert_tokens_to_ids(x) for x in tokenized_wine_texts]
print("None in seqs: ", None in seqs )
...
for seq in seqs:
    if len(seq)>MAX_LEN:
        seq = seq[:(MAX_LEN - 1)].append(seq[-1])
    if not seq:
        #print("seq is none, ", seq)
        none_count +=1
    inpt_seqs.append(seq)
    

# Pad our input tokens
inpt_seqs = pad_sequences(inpt_seqs, maxlen=MAX_LEN, dtype="long", truncating="post", padding="post")

# Create attention masks
attention_masks = []

# Create a mask of 1s for each token followed by 0s for padding
for seq in inpt_seqs:
...
train_inputs, validation_inputs, train_labels, validation_labels = train_test_split(inpt_seqs, varietal_list, 
                                                            random_state=2018, test_size=0.1)
train_masks, validation_masks, _, _ = train_test_split(attention_masks, inpt_seqs,
                                             random_state=2018, test_size=0.1)
